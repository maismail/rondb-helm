...
We use `rclone` to upload backups to object storage. `Rclone` is installed in the `hopsworks/hwutils` image and it claims to work for *a lot* of different storages, including OVH. To add support for a new object storage, you need to add a configuration setting to rclone. For S3, this can look as follows:
...
An easy way of creating this file is to run `docker run --rm -it --entrypoint=/bin/bash rclone/rclone:latest` which will open an terminal in a rclone image. There you can run:
...
- Place "backups" into values.schema.json and add enum for object storage
- Restore from backup
